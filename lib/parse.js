...
module.exports = function ( source, options ) {
	...
	function replaceExport ( node, nodeIndex ) {
		var index, declarations = [], value, ast;

		index = numExports - exportCount++;

		if ( node.declaration ) {
			value = body.splice( nodeIndex + 1, 1 )[0];
			declarations.push( ( options.defaultOnly ? '__export' : 'exports.default' ) + ' = ' + recast.print( value ).code + ( value.type === 'FunctionDeclaration' ? ';' : '' ) );
		}
